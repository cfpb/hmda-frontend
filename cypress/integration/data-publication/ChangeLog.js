...
describe('Change Log', () => {
  describe('Filter Bar', () => {
    it('Loads config from query string', () => {
      ...
      cy.get('.pub-change-item').should('have.length', 2)
      cy.get('.pub-change-item').find('.highlighted').its('length').should('be.gte', 2)

      cy.get('.reset-filters').click()
      cy.get('.pub-change-item').should('have.length', entries.length + 1)

      // Changing filters should update the URL
      cy.get('#filter-bar').findByText('notice').click()
      cy.url().should('contain', '?type=notice')
      cy.get('.pub-change-item:not(.header)').should('not.exist')
