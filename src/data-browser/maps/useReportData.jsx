...
const getFeatureName = (geography, feature) => {
  ...
  const stateAbbr = geography === 'county' ? `, ${fips2Shortcode[feature.substr(0,2)]}` : ''
...
export const gatherReportData = (geoLevel, rawFeature, baseData, combinedFilter1, filter1Data, combinedFilter2, filter2Data) => {
  ...
  if (geoBaseData) {
    ...
    obj.filter1_geo_label = `Applications by ${filter1.variable.label} in ${featureName}`
  }
  ...
  if (geoBaseData) {
    ...
    obj.filter2_geo_label = `Applications by ${filter2.variable.label} in ${featureName}`
  }
  ...
  if (!F1Data) {
    ...
    obj.v2_where_f1_label = `Applications by ${filter2.variable.label} where ${filter1.variable.label} - ${filter1.value.label}`
  ...
  if (!F2Data) {
    ...
    obj.v1_where_f2_label = `Applications by ${filter1.variable.label} where ${filter2.variable.label} - ${filter2.value.label}`
...
export const useReportData = (geoLevel, rawFeature, baseData, combinedFilter1, filter1Data, combinedFilter2, filter2Data) => {
  const [data, setData] = useState()

  useEffect(() => {
    setData(gatherReportData(geoLevel, rawFeature, baseData, combinedFilter1, filter1Data, combinedFilter2, filter2Data))
  }, [geoLevel, rawFeature, baseData, combinedFilter1, filter1Data, combinedFilter2, filter2Data])
