...
it('makes CSV name without vars', () => {
  const csv = makeCSVName({items:['a','b'], variables:{c:{d:123}}}, false)
...
it('calls runFetch and makeUrl on getSubsetDetails', done => {
  ...
  getSubsetDetails({category:'states', items:['abc'],  variables: {a:{b:123}}}).then(data => {
...
it('calls getCSV on makeURL and makeCSVName on getItemCSV call, preventing variab
le inclusion', () => {
  const setAttribute = jest.fn()
  const click = jest.fn()
  ...
  getItemCSV({category: 'states', items:['abc'], variables: {a:{b:123}}})
...
it('calls getCSV on makeURL and makeCSVName on getSubsetCSV call', () => {
  ...
  getSubsetCSV({category: 'states', items:['abc'], variables: {a:{b:123}}})
