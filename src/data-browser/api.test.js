...
it('makes a url', () => {
  const url = makeUrl({states:['abc'], msamds:[], variables: {a:{b:123}}})
  expect(url).toBe('/v2/data-browser-api/view/aggregations?states=abc&a=b&years=2018')
})

it('bails making a url on bad geo input', () => {
...
it('makes a csv url', () => {
  const url = makeUrl({states:['a'], msamds:['b']}, true)
  expect(url).toBe('/v2/data-browser-api/view/csv?states=a&msamds=b&years=2018')
})

it('runs the fetch', done => {
  window.fetch = jest.fn((url, options) => {
    return Promise.resolve({status: 200, json: () => {return {url, options}}})
  })
...
it('makes CSV name', () => {
  const csv = makeCSVName({states:['a','b'], variables:{c:{d:123}}})
  expect(csv).toBe('a,b-c.csv')
})

it('makes CSV name without vars', () => {
  const csv = makeCSVName({states:['a','b'], variables:{c:{d:123}}}, false)
...
it('makes nationwide CSV name', () => {
  const csv = makeCSVName({nationwide: true})
...
it('calls runFetch and makeUrl on getSubsetDetails', done => {
  ...
  getSubsetDetails({states:['abc'], msamds:[], variables: {a:{b:123}}}).then(data => {
    expect(data.url).toBe('/v2/data-browser-api/view/aggregations?states=abc&a=b&years=2018')
    done()
  })
})
...
it('calls getCSV on makeURL and makeCSVName on getSubsetCSV call', () => {
  ...
  getSubsetCSV({states:['abc'], msamds:[], variables: {a:{b:123}}})
