...
import { buildEnumeratedOptions, getFieldType } from './parsedHelpers'
    ...
    >
      ...
      <td className={'fieldName ' + highlightClass}>
        <label htmlFor={fieldName}>
          <Accordion
            content={<FieldDetails field={column} />}
            heading={fieldName}
            id={`${fieldIndex}`}
          />
        </label>
  ...
  // Field allows freeform text but also has enumerated values
    ...
    const derivedInputField = _col.fieldName.includes('Date') ? (
      <input
        ...
        value={formatDateString(_row[_col.fieldName]?.toString() || '')}
      />
    ) : (
      <input
        className='text-input'
        {...common}
        type={getFieldType(_col)}
      ...
      <div className='enum-entry'>
        ...
        <span className='enums'>
          ...
          {enumerations.length >= 3 ? (
            <select {...common} value={creditScoreInputHandler(_row, _col)}>
