...
export const buildCSVRowsAggregate1 = (report) => {
  ...
  _data.forEach((tract) => {
    ...
    tract.dispositions
      .sort(function (x, y) {
        const xp = x.titleForSorting.substring(x.titleForSorting.length - 3)
        const yp = y.titleForSorting.substring(y.titleForSorting.length - 3)
      ...
      .forEach((dispo) => {
        ...
        const _cols = dispo.values
          .sort(function (x, y) {
            const xp = x.dispositionName.substring(x.dispositionName.length - 3)
            const yp = y.dispositionName.substring(y.dispositionName.length - 3)
            return xp == yp ? 0 : xp < yp ? -1 : 1
          })
