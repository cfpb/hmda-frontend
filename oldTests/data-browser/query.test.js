import {
  ...
  makeStateFromSearch,
  makeSearchFromState,
} from '../../src/data-browser/query.js'
...
it('makes variables param from state', () => {
  expect(
    makeParam(
      {
        variables: { a: { b: 123, bb: 1123 }, c: { d: 234 } },
        orderedVariables: ['a', 'c'],
      },
      'variables',
    ),
  ).toBe('a=b,bb&c=d')
...
it('makes variables param from state when unmatched length', () => {
  expect(
    makeParam(
      { variables: { a: { b: 123 } }, orderedVariables: [] },
      'variables',
    ),
  ).toBe('')
})
...
it('makes state from search', () => {
  const state = makeStateFromSearch('?category=states&items=CA', {
    items: [],
    category: '',
  })
...
it('regenerates search when array gets sanitized', (done) => {
  const regen = jest.fn()
  const state = makeStateFromSearch(
    '?category=states&items=CA,XZ',
    { category: '', items: [] },
    null,
    regen,
  )
...
it('makes search with variables', () => {
  const regen = jest.fn()
  const state = makeStateFromSearch(
    '?category=states&items=CA,XZ&actions_taken=2,3&loan_types=',
    { category: '', items: [], variables: {}, orderedVariables: [] },
    null,
    regen,
  )
  expect(state).toEqual({
    category: 'states',
    items: ['CA'],
    orderedVariables: ['actions_taken', 'loan_types'],
    variables: { actions_taken: { 2: true, 3: true }, loan_types: {} },
  })
})
...
it('calls detailsCb when getDetails is present', (done) => {
  ...
  const state = makeStateFromSearch(
    '?category=states&items=CA&getDetails=1',
    { category: '', items: [] },
    detailsCb,
  )
  setTimeout(() => {
...
it('makes search from state', () => {
  const search = makeSearchFromState({
    category: 'states',
    items: ['CA'],
    details: { a: 'b' },
    variables: {},
